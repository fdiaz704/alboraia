...
// Lectura del JSON con todos los tótems instalados
  ...
  // Incorpora los map markers de marquesinas a la lista de map markers
...
Future<void> getPolyline() async {
  // Lectura json con ruta alboraya

  WidgetsFlutterBinding.ensureInitialized();

  String data2 = await rootBundle.loadString('assets/recorrido.json');
  jsonResult = json.decode(data2);

  double latitudOrigen = 0.0;
  double longitudOrigen = 0.0;
  double cont = 1;

  // Genera los polylines de la ruta

  for (var ruta in jsonResult) {
    if (cont % 2 == 0) {
...
class _ScreenState extends State<Screen> {
  ...
  Future<void> pruebaHttp() async {
    var request = http.Request(
        'POST',
        Uri.parse(
            'https://smalboraia.r3smartcity.es/SAE_Alboraya/Bus_info/key.php'));

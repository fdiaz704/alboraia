...
class BddControlerr {
  Future<List<dynamic>> getData(apbid, elect) async {
    ...
    var results =
    await conn.query('UPDATE PLAZAS SET pla_swocupado = 1, pla_email = "amesnau@gmail.com", pla_taraplicable = 1, pla_tariff = 1 WHERE pla_apbid = $apbid AND pla_id = $plaza');
    results =
    await conn.query('SELECT apbi_cargadisp FROM APARCABICIS WHERE apbi_id = $apbid');
    lista = results.toList();
    int disponible = lista[0]["apbi_cargadisp"] - 1;
    results =
    await conn.query('UPDATE APARCABICIS SET apbi_cargadisp = $disponible WHERE apbi_id = $apbid');
